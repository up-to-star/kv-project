...
func TestOpenDataFile(t *testing.T) {
	openDataFile, err := OpenDataFile(os.TempDir(), 0, fio.StandardFileIO)
	assert.Nil(t, err)
	assert.NotNil(t, openDataFile)

	openDataFile1, err1 := OpenDataFile(os.TempDir(), 1, fio.StandardFileIO)
	assert.Nil(t, err1)
	assert.NotNil(t, openDataFile1)
	t.Log(os.TempDir())

	openDataFile2, err2 := OpenDataFile(os.TempDir(), 1, fio.StandardFileIO)
	assert.Nil(t, err2)
...
func TestDataFile_Sync(t *testing.T) {
	openDataFile, err := OpenDataFile(os.TempDir(), 123, fio.StandardFileIO)
	assert.Nil(t, err)
	assert.NotNil(t, openDataFile)
...
func TestDataFile_ReadLogRecord(t *testing.T) {
	openDataFile, err := OpenDataFile(os.TempDir(), 888, fio.StandardFileIO)
	assert.Nil(t, err)
	assert.NotNil(t, openDataFile)
